...
def display_dataset_overview(df,cat_columns,num_columns):
    ...
    st.write(f"**Duplicates:** {st.session_state.new_df.shape[0] - st.session_state.new_df.drop_duplicates().shape[0]}")
    # df_to_use = st.session_state.new_df if "new_df" in st.session_state else df
    # st.write(f"**Duplicates:** {df_to_use.shape[0] - df_to_use.drop_duplicates().shape[0]}")
